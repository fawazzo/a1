...
const RestaurantDetailPage = ({ addToCart }) => {
  const { id } = useParams();
  ...
  const [reviews, setReviews] = useState([
    ...
    },
    {
      ...
      userName: 'Fatma D.',
      rating: 4,
      comment: 'Genel olarak iyi ama biraz daha soslu olabilirdi.',
      date: '2025-11-14',
      verified: true
    },
    {
      ...
      userName: 'Mehmet Y.',
      rating: 5,
      comment: 'En iyi pizzacı! Her zaman sipariş veriyorum.',
      date: '2025-11-13',
      verified: true
    }
  ]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const [resRestaurant, resDishes] = await Promise.all([
        ...
        if (!resRestaurant.ok) {
          throw new Error(restaurantData?.message || 'Failed to load restaurant');
        }
        if (!resDishes.ok) {
          throw new Error(dishesData?.message || 'Failed to load dishes');
        }
